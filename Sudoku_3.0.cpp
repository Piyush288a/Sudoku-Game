...
class Sudoku
{
    ...
    public:
        Sudoku(){};
        Sudoku(string diff,int lvl):level(lvl),difficulty(diff){};
...
void displayBoard(const vector<vector<int>>& board,Sudoku* game) //DISPLAY THE BOARD
{
    ...
    for (int i = 0; i < 9; ++i) 
    {
        ...
        else 
        {
            ...
            {
                std::cout << "     Mistakes : " << game->getMistakeCount() << "/5" << std::endl;
            } 
            else if (i == 4) 
            {
            ...
            {
                std::cout << "     Undo : -1 -1 -1     Exit : 0 0 0";
            } 
            else
            {
                std::cout<<endl;
...
void displayRules() {
    ...
    std::cout << "   - Easy: 10 minutes\n";
    std::cout << "   - Medium: 7 minutes\n";
    std::cout << "   - Hard: 5 minutes\n";
...
bool playGame(Sudoku *game)
{
    ...
    std::cout<<"GAME STARTS! You have limited time to solve the sudoku puzzle.\n";
    ...
    if (game->getDifficulty() == "easy") 
        ...
        (game->getDifficulty() == "medium") totalMinutes = 11;
    else 
        totalMinutes = 20;
    ...
    while(gameOver!=true) //GAME CONTINUES UNTIL 5 MISTAKES OR SOLVED
    {
        ...
        {
            std::cout<<"Undoing the sudoku board..."<<endl;
            if(game->undoMove())
            {
...
int main()
{
    ...
    std::cout<<"Want to start the game? (y/n) : ";             // ASK FOR STARTING THE GAME
    string start;
    cin>> start;
    ...
    {
        std::cout << "Choose difficulty (easy/medium/hard) : ";//ASK FOR DIFFICULTY LEVEL
        string difficulty;
        cin >> difficulty;
        ...
        std::cout << "\nDo you want to play again? (y/n) : "; //ASK FOR REPLAY OR QUIT
        cin >> start;
    }
